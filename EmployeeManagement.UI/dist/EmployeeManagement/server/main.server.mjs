...
function requireNodeUtils() {
  ...
  function serializedTagName(node2) {
    var ns = node2.namespaceURI;
    return ns === NAMESPACE.HTML || ns === NAMESPACE.SVG || ns === NAMESPACE.MATHML ? node2.localName : node2.tagName;
  }
  function fallbackRawContentTags(node2) {
    const tags = [];
    while (node2) {
      if (node2.nodeType === 1) {
        const tagname = serializedTagName(node2);
        if (tagname && (hasRawContentFallback[tagname.toUpperCase()] || hasEscapableRawContent[tagname.toUpperCase()])) {
          tags.push(tagname);
        }
...
var routes = [
  ...
  __spreadValues({
    path: "employees",
    loadChildren: () => import("./chunk-VP6RE5W4.mjs").then((module) => module.EMPLOYEES_ROUTES)
  }, true ? { \u0275entryName: "src/app/features/employees/employees.routes.ts" } : {}),
